...
| Dataset generator creates correctly labelled Reference/Search pairs and connects to inference/evaluation | Fresh command: `generate_dataset.py --out data/_acceptance_smoke --n 1 --seed 20260817 --arch finfet --rgb`, followed by public `infer.py` and `evaluate.py` | Generator wrote the RGB pair and manifest; inference printed `388.87,405.51`; evaluator consumed it and reported **459.01 px**, 0% at 1--20px, with the sample in the ambiguous subset | Direct end-to-end integration boundary exercised. This is an honest observed ambiguity failure, not a claimed quality success. `results/acceptance_smoke.json`. |
...
| RGB optical bonus | Fresh end-to-end `generate_dataset.py --n 30 --seed 888 --arch mixed --rgb`, then `evaluate.py` | n=30: 60.0% @10px, 0.08px median, 212.7ms/sample; unique n=16 was 100% @10px (0.06px median), ambiguous n=14 was 14.3% @10px | `results/rgb_acceptance_n30_seed888.json`. Synthetic only, and the 1-pair ambiguous failure above is retained as negative evidence. |
